    ...
    spec:
      ...
      containers:
        - name: ray-head
          ...
          args:
          {{ if .Values.workdir }}
            - {{ print "set -e; set -o pipefail; " (include "ray.head.args" .) "; mkdir /tmp/workdir; tar -C /tmp/workdir -jxf /tmp/configmap/workdir/workdir.tar.bz2; if [ ! -f /tmp/workdir/runtime-env.yaml ]; then touch /tmp/workdir/runtime-env.yaml; if [ -f /tmp/workdir/requirements.txt ]; then echo 'Splicing in requirements.txt'; awk 'BEGIN { print \"pip:\" } NF > 0 { print \"  - \" $0 }' /tmp/workdir/requirements.txt >> /tmp/workdir/runtime-env.yaml; fi; fi ; if [ ! -s /tmp/workdir/runtime-env.yaml ]; then echo 'Cleaning out empty runtime-env.yaml'; rm -f /tmp/workdir/runtime-env.yaml; fi; ls -l /tmp/workdir ; stdbuf -o0 -i0 -e0 ray job submit --job-id " (.Values.jobId) " --address http://localhost:8265 --working-dir=/tmp/workdir --runtime-env=/tmp/workdir/runtime-env.yaml -- " (.Values.commandLinePrefix | default "python3 main.py") " " (.Values.dashdash | default "" | b64dec) "; echo 'Job complete, shutting down ray...'; ray stop >& /tmp/ray-stop.out" }}
          {{ else }}
